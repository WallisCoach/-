...
<script>
import {allotNewRoleRequest} from '';
import {getRolesListRequest} from ''
...
export default {
    ...
    methods:{
        ...
        showSetRoleDialog(role){
            this.uesrInfo = role;
            //获取所有的角色列表
            getRolesListRequest().then(res)(res=>{
                let result =- res.data;
                if (result.meta.status !== 200){
                    return this.alertMessage('获取角色列表失败','error');
                }
                this.rolesList = result.datal
            });
        ...
        allotRole(){
            ...
            allotNewRoleRequest(this.uesrInfo.id,this.selectRoleId).then(res =>{
              let result = res.data;
